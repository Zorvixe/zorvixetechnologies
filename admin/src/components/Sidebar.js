...
export default function Sidebar({ current, onNavigate, collapsed }) {
  ...
  const goto = (key) => (e) => { e.preventDefault(); onNavigate(key) }
  ...
  return (
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      <nav className="sidebar-nav">
        ...
        <a href="#" className={`navlink ${current === 'stats' ? 'active' : ''}`} onClick={goto('stats')} title="Stats">
        ...
        <a href="#" className={`navlink ${current === 'clients' ? 'active' : ''}`} onClick={goto('clients')} title="Clients">
        ...
        {user?.role === 'admin' && (
          <a href="#" className={`navlink ${current === 'payments' ? 'active' : ''}`} onClick={goto('payments')} title="Payments">
            {/* icon */}<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2" /><line x1="2" y1="10" x2="22" y2="10" /><path d="M6 16h2" /><path d="M10 16h6" /></svg>
        ...
        {user?.role === 'admin' && (
          <a href="#" className={`navlink ${current === 'users' ? 'active' : ''}`} onClick={goto('users')} title="Users">
            {/* User icon */}
        ...
        {user?.role === 'admin' && (
          <a href="#" className={`navlink ${current === 'candidates' ? 'active' : ''}`} onClick={goto('candidates')} title="Onboarding">
        ...
        <a href="#" className={`navlink ${current === 'tickets' ? 'active' : ''}`} onClick={goto('tickets')} title="Tickets">
          {/* icon */}<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"></path><path d="M13 5v2"></path><path d="M13 17v2"></path><path d="M13 11v2"></path></svg>
          {!collapsed && <span>Tickets</span>}
        </a>

        <a href="#" className={`navlink ${current === 'contacts' ? 'active' : ''}`} onClick={goto('contacts')} title="Contacts">
          {/* Address book / contact card icon */}
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
            fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
